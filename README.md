...
3. 填写基础乘区、候选觉醒和武器效果；
4. 记录伤害技能与治疗技能；
5. 选择常规验算或狂暴溯源、调整搜索强度并开始计算。

窗口宽度建议不低于 1600 像素；屏幕较窄时页面会自动换行。
...
5. 治疗技能在独立区域填写，不要放进伤害技能列表。
6. 选择“常规验算”，通常保持“深度”搜索，点击“确定并开始验算”。
...
| 字段 | 单位与含义 | 注意事项 |
| --- | --- | --- |
| 非战斗面板攻击 | 直接填写面板显示值 | 指未进入战斗时的最终面板，不要把候选觉醒攻击加成预先重复算入。 |
| 基础攻击 | 直接填写数值 | 用于把战斗攻击加成换算成额外攻击。 |
| 基础战斗攻击加成 | 百分数，例如 `20` 代表 20% | 战斗攻击为“非战斗面板攻击 + 基础攻击 × 生效攻击加成总和”。 |
| 角色等级 | 整数 | 进入防御区公式。 |
| 敌人等级 | 整数 | 进入防御区公式。 |
| 敌人基础抗性 | 百分数，例如 `20` 代表 20% | 抗性区为 `1 - 基础抗性 + 抗性削弱`。 |

### 3. 基础乘区

基础乘区表示不依赖候选觉醒或武器、当前构筑原本就应存在的数值。所有百分数字段直接输入百分数，不需要输入 `%`。
...
每一行代表一个可以独立判断生效或失效的候选效果。名称应能直接定位来源，例如“觉醒3”“专武二层”“队友光环”。同一行可同时填写多个提升，代表它们共享该行的整体生效状态；程序也会继续检查行内单项提升是否未生效。
...
| 伤害模型 | “常规”使用完整攻击、乘区和倍率公式；“创生”使用固定基础值 9000。 |
...
治疗与伤害分别记录、分别计算。填写治疗倍率和实测显示治疗后勾选“纳入验算”。治疗公式为 `战斗面板攻击 × 治疗倍率`；伤害技能的增伤区和暴击区不会自动作用于治疗。
...
创生伤害不读取面板攻击、常规增伤区或普通技能倍率。更完整的百分数、加算/乘算及失效规则见 [计算公式](docs/formulas.md)。
